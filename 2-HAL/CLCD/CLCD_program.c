...
void CLCD_voidWriteSpecialCharacter(u8 *Copy_pu8Pattern , u8 PatternNumber , u8 Copy_u8XPos , u8 Copy_u8YPos ) {
	...
	/* send to DDRAM to display the pattern  */ 
...
void CLCD_voidWriteFloat(float Copy_f32Number, u8 decimalPlaces) {
	...
	// Round the float to the desired number of decimal places
	Copy_f32Number = roundf(Copy_f32Number * multiplier) / multiplier;

	// Convert the float to an integer (ignoring the decimal part)
	...
	// Calculate and display the decimal part
	s32 decimalPart = (s32)((Copy_f32Number - (float)integerPart) * multiplier);
